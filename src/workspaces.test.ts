...
test("managed worktrees archive without deleting files", async (t) => {
  const context = await fixture(t);
  const gitRoot = await createGitProject(context.root);
  const stateDir = join(context.root, ".archive-state");
  ...
  try {
    ...
    assert.equal(archived.workspaceId, opened.workspace.id);
    assert.equal(archived.root, opened.workspace.root);
